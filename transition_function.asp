...
% merging: non vorrei che, date due celle adiacenti, dello stato "current", con valori uguali, non esista una cella cella
...
merged_cell_y(X, Y, X2, Move) :- cell("current", X, Y, V), cell("current", X2, Y, V),
    &abs(X - X2; Dx), Dx == 1,
    move(Move), cell(Move, _, Y, V2), V2 == V * 2.

:~ move(Move), cell("current", X, Y, _), cell("current", X, Y2, _), not merged_cell_x(X, Y, Y2, Move). [1@2, Y, Y2]
:~ move(Move), cell("current", X, Y, _), cell("current", X2, Y, _), not merged_cell_y(X, Y, X2, Move). [1@2, X, X2]
